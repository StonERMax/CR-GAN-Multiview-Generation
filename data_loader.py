...
def get_multiPIE_img(img_path):
    ...
    # the infro about image
    token = name.split('_')
    ID = token[0]
    status = token[2]
    bright = token[4]

    # image with the same ID, the same illumination but different view
    ...
    # return image with the same ID, the same illumination but different view
    return view2, img2
...
def get_300w_LP_img(img_path):
    ...
    # return the image and its viewpoint
    return view2, img2
