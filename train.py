...
for epoch in range(args.epochs):
    for i, (view1, view2, data1, data2) in enumerate(train_loader):
        # view 1 and data 1 is the image read
        # view 2 and data 2 is the randomly chosen image with different viewpoint
        # our framework:
        ...
        # eps and temp are between 0 and 1
        ...
        # v marks the viewpoint of each image in the array
        ...
        # np.where()在高维矩阵的时候，输出的是坐标
        idxs = np.where(targetNP>0)[1]
        tmp = torch.LongTensor(idxs)
        # vv1 是每一张图片的viewpoint序号，且是一个一维向量
